...
And in the [simple-manager-blueprint-inputs.yaml](https://github.com/cloudify-cosmo/cloudify-manager-blueprints/simple-manager-blueprint-inputs.yaml) set the port to `9201`:
...
invalid states to prevent them from turning up at runtime. This paragraph will describe the validations preformed for each of the services prior to the upgrade workflow
execution.
...
* Elasticsearch
    * `es_heap_size` property must not be smaller than current memory size.
...
### Upgrade Execution
Starting the upgrade process requires having the Cloudify Manager in [maintenance-mode](http://??). This will prevent having running executions during the upgrade process.
...
Manager rollback should be done in-case of failure in the upgrade process or simply due to dissatisfaction from the new upgraded version.
...
1. Rerunning the `cfy upgrade` command again. Maybe try running it with a different configuration. Since the upgrade process is idempotent, it will not have any harmful effect.
2. Running the `cfy rollback` command. This will result in having all services reinstalled in their old version.

{{% gsNote title="Note" %}}
The snapshot used for the upgrade process should not be deleted until it it certain rollback will not be used though it is advised to keep it anyways.
{{% /gsNote %}}
